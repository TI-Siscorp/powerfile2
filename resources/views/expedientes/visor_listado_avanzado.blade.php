...
@section('tituloPagina')
	<h3>{{ trans('principal.titvvisor') }} <span id="titi"></span></h3>
@endsection
...
<?php
	...
	$numimgdoc = $permisos->contarimagenesxdoc($id_documento);   
...
<div class="panel">
	...
	@endif
       ...
       <div id="botones" class="noseve btnvisor">    <!-- sombratabla-->
        	<table width="50%" border="0">
        		<tr>
			        ...
			        <td id="tddescarga" width="5%" style="display:none"> 
					...
					<td width="5%" class="centrartexto">
			          ...
			          <td id="tdzoom" width="10%" style="padding-left:3%"> 
			       		<label>zoom</label><input id="rzoom" type="range" min="0" max="200" value="1" oninput="elzoom(this.id)">&nbsp;&nbsp;
			         </td>
		    ...
		    <div class="modal-dialog" style="width: 70%;overflow-y:auto;overflow-x:none;max-heigth:10%;z-index:99999999 !important;">
		      ...
		      <div class="modal-content">
			        ...
			        <form id="formdocum" method="POST" action="<?php echo $ruta;?>/treepowerfile2/cargalo_documentos.php" accept-charset="UTF-8" class="form-horizontal bpad15L pad15R bordered-row" enctype="multipart/form-data">
			            ...
			            <input id="origen" type="hidden" name="origen" value="visor_listado_avanzado">
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$('#titi').html('{{ $datosmostrar }}&nbsp;&nbsp;({{ $numimgdoc }}&nbsp;&nbsp;{{ trans("principal.titimage") }})');   
		...
		$.ajax({
		   ...
		   success: function(msg){  
			...
			error: function(x,err,msj){alert('lipieza: '+msj) }
		  });*/
	...
	function ejecutarluego(id_documento){ 
		...
		var ruta = '{{ $ruta }}';
		 ...
		 window.location.href = '{{ $ruta }}/expedientes/'+id_documento+'/'+buscar+'/visor_listado';	
	...
	function agregarimagenes(){
		   ...
		   var ntipdoc = $('#titi').text();
		    ...
		    $('#id_documento').val(id_documento);
	   		...
	   		$('#paddimg').attr('src',ruta+"/treepowerfile2/l_agregaimg.php?id_tipodoc="+id_tipodoc+"&id_documento="+id_documento+"&id_expediente="+id_expediente+"&ruta="+ruta+"&id_usuario="+idusuario+"&buscar="+buscar+"&ntipdoc="+ntipdoc+"&vi=a");
	...
	function zoomtodo(id){ 
		...
		$.ajax({
		   ...
		   success: function(msg){   
			...
			error: function(x,err,msj){alert('zoot '+msj) }
		  });
	       ...
	       if (v == 1)
			...
			if ($("#deimagenes").is(":visible") == true) 
				{ 
					...
					var $scriptimg = '<img id="image_central" src="'+docum+'" data-1="'+d1+'"  data-2="'+d2+'" class="sb-toggle-right imgcentral zoom" onclick="zoomimg(this.id,1)" data-exp="'+expe+'" data-num="'+numim+'" alt="" width="80%" max-width="100%" style="cursor:pointer">';
   ...
   function mhori(id){  
	   ...
	   $('#imagen_visual').css("left",mover) ;
		
   } 
   
   function mver(id){
	   var mover = $('#'+id).val()+'%';	   	   	   
	   $('#imagen_visual').css("top",mover) ;
	...
	$('.ver_indices').bind('click', function () {
		...
		$.ajax({
			   ...
			   success: function(msg){ 
				...
				error: function(x,err,msj){alert('indices '+msj) }
			  });
	...
	$('.ver_imagenes').bind('click', function () {
		...
		$.ajax({
			   ...
			   success: function(msg){    
				...
				error: function(x,err,msj){alert('v img '+msj) }
			  });
		...
		if (iddoc != docactivo)
			{
				...
				var script = '';
				var enlacerecep = ruta+'/treepowerfile2/cargalo_documentos.php?otraoperation=dameimagenescentro'+'&id_documento='+iddoc+'&configdb='+configdb+'&workspace='+workspace; 
				$.ajax({
					   type: "GET",
					   async:false, 
					   url: enlacerecep,
					   success: function(msg){    
						...
						error: function(x,err,msj){alert('verimgtp ' +msj) }
					  });
		...
		if (iddoc != docactivo)
			{
				...
				$.ajax({
					   ...
					   success: function(msg){    
						...
						error: function(x,err,msj){alert('verimgtpfull '+msj) }
					  });
	   ...
	   var id_expediente = $('#'+id).attr('data-valor'); 
		...
		$.ajax({
			   ...
			   success: function(msg){    
				...
				error: function(x,err,msj){alert('verimagenes ' +msj) }
			  });
	   ...
	   var enlacerecep = ruta+'/treepowerfile2/cargalo_documentos.php?otraoperation=dameiddocumento'+'&id_expediente='+id_expediente+'&id_tipodoc='+id_tipodoc+'&configdb='+configdb+'&workspace='+workspace; 
		$.ajax({
		   ...
		   success: function(msg){   
			...
			error: function(x,err,msj){alert('agreimg '+msj) }
		  });
	   }
   ...
   function avanzarimg(direccion){  
		...
		var vorde = orde.split('_,_');  // alert(vorde);
   ...
   function grabay(id_documento){
			...
			buscar = buscar.replace(/%/g, '_..._');
			 ...
			 window.location.href = '{{ $ruta }}/'+espaciotrabajo+'/expedientes/'+id_documento+'/'+buscar+'/'+id_tabla+'/visor_listado_avanzado';
	} 	
	 ...
	 function quedescarga(){
	  ...
	  $( "#descargardoc" ).click(function() { 
			...
			if (permiorden == 1)
				...
				{
					script +='<iframe id="print-iframe" name="print-iframe" src="/ViewerJS/./#'+laimg+'#toolbar=0" height="1000" width="80%"></iframe>';
				}	
	  ...
	  $( "#guardardoc" ).click(function() { 
		...
		function visor_dopc(iddocs){
			...
			window.location.href = '{{ $ruta }}/'+espaciotrabajo+'/expedientes/'+id_documento+'/'+buscar+'/'+tablaid+'/visor_listado_avanzado';
