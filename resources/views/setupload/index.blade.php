...
@section('titulo')
 {{ trans('principal.loadti') }}  
@endsection

@section('tituloPagina')
    <h3>{{ trans('principal.loadti') }}  </h3>
@endsection
    ...
    <div class="panel-body">
		...
		@endif
    		...
    		@if ($permisos->conocepermisos('set_up_load') == true)
    			<a href="{{ route('setupload.create') }}" class="btn btn-primary" role="button">{{ trans('principal.nload') }}</a>
    		@endif	
    		<br>
    		<br>
                      ...
                      <thead>
                        ...
                        @foreach($setup as $datos)
                            ...
                            <td class="centrartexto">
                             @if ($permisos->conocepermisos('edit_load') == true)<a href="{{ route('setupload.edit',$datos->id_setup) }}" class="btn btn-primary" role="button" title="{{ trans('principal.msgedit') }}"><span class="glyphicon glyphicon-edit"></span></a>@endif	
                             @if ($permisos->conocepermisos('delete_load') == true)<a  data-b="{{ $datos->id_setup }}" href="{{ route('setupload.destroy',$datos->id_setup) }} " class="btn btn-danger btnborrar" title="{{ trans('principal.msgelim') }}" onclick="return confirm('{{ trans('principal.msgconfelimload') }}')" role="button" ><span class="glyphicon glyphicon-trash"></span></a>@endif
