...
@section('titulo')
 {{ trans('principal.bodega') }}  
@endsection

@section('tituloPagina')
    <h3>{{ trans('principal.bodega') }}  </h3>
@endsection
...
<div class="panel">
    <div class="panel-body">
    		...
    		@if ($permisos->conocepermisos('set_up_storage') == true)
    			<a href="{{ route('setupbodega.create') }}" class="btn btn-primary" role="button">{{ trans('principal.nstorage') }}</a>
    		@endif	
    		<br>
    		<br>
                      ...
                      <thead>
                        ...
                        @foreach($setup as $datos)
                            ...
                            <td class="centrartexto">
                             @if ($permisos->conocepermisos('edit_storage') == true)<a href="{{ route('setupbodega.edit',$datos->id_setup) }}" class="btn btn-primary" role="button" title="{{ trans('principal.msgedit') }}"><span class="glyphicon glyphicon-edit"></span></a>@endif	
                             @if ($permisos->conocepermisos('delete_storage') == true)<a  data-b="{{ $datos->id_setup }}" href="{{ route('setupbodega.destroy',$datos->id_setup) }} " class="btn btn-danger btnborrar" title="{{ trans('principal.msgelim') }}" onclick="return confirm('{{ trans('principal.msgconfelimbod') }}')" role="button" ><span class="glyphicon glyphicon-trash"></span></a>@endif
