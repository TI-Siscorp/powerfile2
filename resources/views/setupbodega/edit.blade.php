...
@section('titulo')
 {{ trans('principal.editbodega') }} 
@endsection

@section('tituloPagina')
<h3>{{ trans('principal.editbodega') }} : {{ $bodegas->ftp_server }}</h3>
@endsection
...
<div class="panel">
	<div class="panel-body">
		...
		{!! Form::model($bodegas, ['method' => 'PATCH','route' => ['setupbodega.update', $bodegas->id_setup],'class'=>'form-horizontal bordered-row']) !!}
	
	
		    <div class="form-group">
	             {!! Form::label('modobodega',trans('principal.modostorage'),['class'=>'col-sm-3 control-label']) !!}
	             <div class="col-sm-6">
		    	...
		    	<div class="form-group mododebodega">
					...
					<div class="col-sm-6">
						{!! Form::text('ftp_server',$bodegas->ftp_server,['class'=>'form-control']) !!} 
					</div>
				</div>
				...
				<div class="form-group mododebodega">
					...
					<div class="col-sm-6">
						{!! Form::text('ftp_user',$bodegas->ftp_user,['class'=>'form-control']) !!}
					</div>
				</div>
				...
				<div class="form-group mododebodega">
                   {!! Form::label('ftp_pass',trans('principal.itempassserver'),['class'=>'col-sm-3 control-label']) !!}
                    <div class="col-sm-6">
                       {!! Form::text('ftp_pass',$bodegas->ftp_pass,['class'=>'form-control']) !!}
                    </div>
           		</div>	
				...
				<div class="form-group mododebodega">
					...
					<div class="col-sm-6">
						{!! Form::text('ftp_port',$bodegas->ftp_port,['class'=>'form-control']) !!} 
					</div>
				</div>
		    ...
		    <div class="form-group">
				...
				<div class="form-group centrartexto" >
					...
					{{ link_to_route('setupbodega.index', trans('principal.btnca'), '', array('class'=>'btn btn-danger btn-close')) }}
				</div>
...
<script type="text/javascript">		
	...
	var id_estadoreg = '{{ $bodegas->id_estado }}'; 

	var estatus = '{{ $bodegas->estatus }}'; 

	var modobodega = '{{ $bodegas->modobodega }}';   
