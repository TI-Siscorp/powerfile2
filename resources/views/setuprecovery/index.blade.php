...
@section('titulo')
 {{ trans('principal.msgconfrecovery') }}  
@endsection

@section('tituloPagina')
    <h3>{{ trans('principal.msgconfrecovery') }}  </h3>
@endsection
    ...
    <div class="panel-body">
		...
		@endif
    		...
    		@if ($permisos->conocepermisos('set_up_load') == true)
    			<a href="{{ route('setuprecovery.create') }}" class="btn btn-primary" role="button">{{ trans('principal.nrecovery') }}</a>
    		@endif	
    		<br>
    		<br>
                      ...
                      <thead>
                        ...
                        @foreach($setup as $datos)
                            ...
                            <td class="centrartexto">
                             @if ($permisos->conocepermisos('edit_recovery') == true)<a href="{{ route('setuprecovery.edit',$datos->id_setup) }}" class="btn btn-primary" role="button" title="{{ trans('principal.msgedit') }}"><span class="glyphicon glyphicon-edit"></span></a>@endif	
                             @if ($permisos->conocepermisos('delete_recovery') == true)<a  data-b="{{ $datos->id_setup }}" href="{{ route('setuprecovery.destroy',$datos->id_setup) }} " class="btn btn-danger btnborrar" title="{{ trans('principal.msgelim') }}" onclick="return confirm('{{ trans('principal.msgconfelimerecovery') }}')" role="button" ><span class="glyphicon glyphicon-trash"></span></a>@endif
