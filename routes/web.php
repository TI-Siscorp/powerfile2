...
function varia(){
	...
	{	
		...
		{	
			if (@$_SESSION['espaciotrabajo'] == null)
				{
					$titr = explode("/",$_SERVER['REQUEST_URI']);
					echo '<script language="javascript">window.location="'.$ruta.'/'.$titr[1].'/"</script>';
				}
			else
				{
...
Route::group(['middleware' => ['web']], function () {
	...
	Route::get($mespacio.'expedientes/llenasearch',[
			'uses' => $elespacio.'\ExpedientesController@llenasearch',
			'as' => 'expedientes.llenasearch'
	]);
	
	Route::get($mespacio.'llenasearch',[
			'uses' => $elespacio.'\ExpedientesController@llenasearch',
			'as' => 'llenasearch.llenasearch'
	]);
	...
	//rutas para crud setup de bodegas
	Route::resource($mespacio.'setupbodega',$elespacio.'\SetupbodegaController');
	
	//rutas para crud setup de load
	Route::resource($mespacio.'setupload',$elespacio.'\SetuploadController');
	
	//rutas para crud setup de error
	Route::resource($mespacio.'setuperror',$elespacio.'\SetuperrorController');
	
	//rutas para crud setup de recovery
	Route::resource($mespacio.'setuprecovery',$elespacio.'\SetuprecoveryController');
	
	
	Route::get($mespacio.'setupbodega/{bodega}/destroy',[
			'uses' => $elespacio.'\SetupbodegaController@destroy',
			'as' => 'setupbodega.destroy'
	]);
	
	Route::get($mespacio.'setupload/{bodega}/destroy',[
			'uses' => $elespacio.'\SetuploadController@destroy',
			'as' => 'setupload.destroy'
	]);
	
	Route::get($mespacio.'setuperror/{bodega}/destroy',[
			'uses' => $elespacio.'\SetuperrorController@destroy',
			'as' => 'setuperror.destroy'
	]);	
	
	Route::get($mespacio.'setuprecovery/{bodega}/destroy',[
			'uses' => $elespacio.'\SetuprecoveryController@destroy',
			'as' => 'setuprecovery.destroy'
	]);
	...
	Route::post($mespacio.'api/documentos/creadocb64',[
			'uses' => $elespacio.'\Api\ApidocumentosController@creadocb64',
			'as' => 'documentos.creadocb64'
	]);
...
<<<<<<< web.php
		...
		Route::get('leaseplan/', function () {
				return view('auth.login');
		});

		Route::get('jlt/', function () {
				return view('auth.login');
		});
=======
	>>>>>>> 1.89
