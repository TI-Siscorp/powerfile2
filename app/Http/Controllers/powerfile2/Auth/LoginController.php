...
namespace App\Http\Controllers\powerfile2\Auth;
use Lang;
use App\Usuario;
use App\Logo;
use Validator;
		...
		if (auth()->attempt(['login' => trim($login), 'password' => trim($clave)]))
			{
				...
				//se completa la tabla search con el fulltext
				
				
				$damelo = DB::select("SHOW INDEX FROM sgd_search");   
				
				$sihayfull = 0;
				
				for ( $i = 0 ; $i < count($damelo); $i ++)
					{
							if ($damelo[$i]->Key_name == 'full')
								{
					...
					if ($sihayfull == 0)
						{
							...
							@DB::statement('ALTER TABLE sgd_search ADD FULLTEXT full(nombre, search)');
						}
