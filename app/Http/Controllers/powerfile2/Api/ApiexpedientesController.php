									...
									if ($buscar != '_;_')
										{
											if ($driver != 'pgsql')
												{
													$expedientes = DB::select("SELECT * FROM ".$workspace.".sgd_search WHERE MATCH(nombre,search)  AGAINST ('$buscar*' IN BOOLEAN MODE)");  
												}
											else
												{
													if ($driver == 'pgsql')
														{
															$expedientes = DB::select("SELECT * FROM ".$workspace.".public.sgd_search WHERE MATCH(nombre,search)  AGAINST ('$buscar*' IN BOOLEAN MODE)");  
							...
							if ($registroper == true)
								{
									...
									if (count($documentos) == 0)
										...
										{
											return response()->json((['status'=>'error','code'=>201,'message'=>'The file can not be deleted, it contains registered documents!']));
										}
								}
							else
								{
