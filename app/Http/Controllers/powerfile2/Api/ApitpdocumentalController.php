						...
						if ($driver != 'pgsql')
							{
								...
								$tipos = DB::select('select id_tipodoc_indice from '.$workspace.'.sgd_tipodoc_indices where id_tipodoc = '.$id);
								
								if (count($tipos) == 0)
									{
										$tiposd = DB::select('select id_documento from '.$workspace.'.sgd_documentos where id_tipodocumental = '.$id);
										...
										if (count($tiposd) == 0)
											...
											{
												return response()->json((['status'=>'error','code'=>201,'message'=>'Document type has registered documents, can not be deleted']));
											}
									}
								else
									{
										return response()->json((['status'=>'error','code'=>201,'message'=>'The document type has registered data, can not be deleted']));
									}
						...
						else
							{
								if ($driver == 'pgsql')
									{
										//se verifica si el tipo documental tiene carga
										$tipos = DB::select('select id_tipodoc_indice from '.$workspace.'.public.sgd_tipodoc_indices where id_tipodoc = '.$id);
										
										if (count($tipos) == 0)
											{
												$tiposd = DB::select('select id_documento from '.$workspace.'.public.sgd_documentos where id_tipodocumental = '.$id);
