								...
								if ($driver != 'pgsql')
									{
										...
										//se verifica si tiene nodos relacionados
										
										$depend = DB::select("select id_dependen_folder from '.$workspace.'.sgd_dependencias_folders where id_tabla = ".$id);
												...
												if (count($depend) > 0)
													{
														...
														return response()->json((['status'=>'error','code'=>201,'message'=>'The unit has registered nodes, can not be deleted']));
										...
										if ($driver == 'pgsql')
											{
												...
												$depend = DB::select("select id_dependen_folder from '.$workspace.'.public.sgd_dependencias_folders where id_tabla = ".$id);
