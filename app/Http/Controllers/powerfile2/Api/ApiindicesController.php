					...
					if ($registroper == true)
						{
							if ($driver != 'pgsql')
								{
									//se verifica si el indice tiene datos amarrados
									
									$indices = DB::select('select id_valorindice from '.$workspace.'.sgd_valorindice where id_indice = '.$id);
									...
									if (count($indices) == 0)
									...
									else
										{
											if (count($indices) > 0)
												{
													return response()->json((['status'=>'error','code'=>201,'message'=>'Index has logged data, can not be deleted']));
												}
										}
								}
							else
									...
									if ($driver == 'pgsql')
										{
											...
											$indices = DB::select('select id_valorindice from '.$workspace.'.public.sgd_valorindice where id_indice = '.$id);
